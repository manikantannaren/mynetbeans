...
@NbBundle.Messages({
    "displayName=Generate serialVersionUID"
})
public class SUIDGenerator implements CodeGenerator {
    ...
    public void invoke() {
        try {
            ...
            CancellableTask<WorkingCopy> task = new SUIDGenCancellableTask();
            ModificationResult result = source.runModificationTask(task);
            result.commit();
        } catch (IOException ex) {
            Logger.getLogger(SUIDGenerator.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private class SUIDGenCancellableTask implements CancellableTask<WorkingCopy> {
        ...
        public void run(WorkingCopy workingCopy) throws Exception {
            ...
            List<Tree> treeDeclarations = (List<Tree>) compilationUnitTree.getTypeDecls();
            ...
            if (clazz != null) {
                Set<Modifier> modifiersSet = new TreeSet<Modifier>();
                ...
                VariableTree suidVarTree = maker.Variable(modifiers, "serialVersionUID", longTypeTree, initialValueTree);
                ClassTree modifiedClazz = maker.addClassMember(clazz, suidVarTree);
